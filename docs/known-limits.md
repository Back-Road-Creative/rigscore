...
| Assurance about your **home** skill library (`~/.claude/skills`, `~/.codex/prompts`, …) | Home-scope skill/command scanning is **off by default**: `--include-home-skills` gates it, so a green *project* scan says nothing about cross-project injection sitting in your home dirs. | Run `rigscore --include-home-skills`, or scan `~` directly, on a cadence of its own. |
