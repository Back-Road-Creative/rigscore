...
Scans every skill / command / sub-agent instruction file visible to the project — all governance files other than `CLAUDE.md`, plus every file under the project-level command/skill dirs (recursive) — for patterns that would hijack, exfiltrate, escalate, persist, or disguise instructions. Those dirs are **derived from the client registry** (`src/clients.js` `skillDirs`), not hardcoded: `.claude/commands/`, `.claude/skills/`, `.opencode/commands/`, and `.gemini/commands/`. Optionally extends to every registered client's HOME dir (`~/.claude/commands/**`, `~/.claude/skills/**`, `~/.codex/prompts/**`, `~/.config/opencode/commands/**`, `~/.gemini/commands/**`) via `--include-home-skills`. Maps to OWASP Agentic Top 10 `ASI01` (Agent Goal Hijack). A passing check guarantees: no instruction-override patterns (single-line or 2-line sliding window), no shell execution directives, no outbound data exfiltration, no privilege escalation, no persistence instructions, no indirect injection (`eval`, `new Function`, "download and run"), no trust-exploitation phrases (CVE-2025-54136 class), no Unicode steganography (bidi overrides, zero-width, homoglyphs including Mathematical Bold / Fullwidth Latin / Cherokee), no plaintext HTTP URLs, no suspicious base64 blobs, and no world-writable skill files.
...
|---|---|
| `sudo` as a whole word — **case-sensitive** | `skill-files/escalation-sudo` |
| `run as root` | `skill-files/escalation-run-as-root` |
| `run as admin` | `skill-files/escalation-run-as-admin` |
| `elevate` / `elevated` followed by `privileg…` | `skill-files/escalation-elevated-privilege` |
| `chmod 777` — **case-sensitive** | `skill-files/escalation-chmod-777` |
| `chmod +x` — **case-sensitive** | `skill-files/escalation-chmod-plus-x` |
| `chmod` followed by any `a+` mode grant (e.g. `chmod a+rwx`) — **case-sensitive** | `skill-files/escalation-chmod-a-plus` |
...
- Default scope: project `cwd` only — the `base: 'cwd'` skill dirs of every registered client (`.claude/commands/`, `.claude/skills/`, `.opencode/commands/`, `.gemini/commands/`). `--include-home-skills` opts in to scanning each client's `base: 'home'` dir (`~/.claude/commands/**`, `~/.claude/skills/**`, `~/.codex/prompts/**`, `~/.config/opencode/commands/**`, `~/.gemini/commands/**`) — disabled by default to keep project scores independent of per-user skill libraries. A client that declares no `skillDirs` surface contributes no dirs.
