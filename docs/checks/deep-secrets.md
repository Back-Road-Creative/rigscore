...
| File walker reached the configured cap (`deepScan.maxFiles`, default 1000) and stopped early — files past the cap were never scanned, so the result cannot be read as "no secrets". Suppresses the PASS finding | WARNING | `deep-secrets/file-cap-reached` | Raise `deepScan.maxFiles` in `.rigscorerc.json` or narrow the scan root |
