...
> **Grade caveat — `graduated-script-missing` is not keyword-graded.** `enforcementGrade` is declared per check, and the label above is accurate for sub-checks 1–5. Sub-check 0 is **mechanical**: it resolves the script path a `status: graduated-code` skill invokes and asks the filesystem whether it exists. That is binary and not evadable by rewording — the property THREAT-MODEL.md calls "load-bearing" and tells auditors to trust, as opposed to `keyword` signal it tells them to discount. Treat this one finding as mechanical despite the check-level label. Giving it the grade it deserves needs either per-finding grading or its own check; both are larger changes than the sub-check itself, and neither is done.

## Purpose

Measures how well a project's AI-dev artifacts (skills, MCP servers, memory, pipelines) line up with graduation and decomposition signals from the AI development taxonomy (thresholds are inlined in `src/checks/workflow-maturity.js` — see `## Scope and limitations` below). Six sub-checks: (0) **graduated-script-missing** — a skill whose frontmatter declares `status: graduated-code` asserts it is a thin wrapper over a deterministic script; if that script isn't on disk the skill is non-functional (`exit 127` on every invocation) while its version bump advertises the opposite. (1) **eval-coverage** — every discovered skill should have at least one eval or test that actually contains something, since graduation from skill → code requires coverage; an eval directory that is empty or holds only a README reports separately as `skill-eval-hollow`. (2) **skill-compound-responsibility** — skills with ≥8 trigger keywords likely handle multiple concerns and should be split. (3) **mcp-single-consumer** — an MCP server referenced by ≤1 skill doesn't justify the MCP overhead versus a library. (4) **memory-orphan** — `.md` files in a memory directory that aren't linked from `MEMORY.md` bias the model silently. (5) **pipeline-step-overload** — Python pipeline files with ≥10 stage markers, or stage/phase directories with ≥10 modules, indicate orchestration breadth that warrants sub-pipeline decomposition. Maps to OWASP Agentic Top 10 **ASI01 — Agent Authorization & Control Hijacking** loosely (sprawling, unevaluated agents are harder to constrain), but the primary frame is maturity and refactor signal, not vulnerability.
...
| One of those exists but carries no runnable coverage — an empty or README-only `evals/<name>/`, or an empty `tests/test_<name>*` | INFO | `workflow-maturity/skill-eval-hollow` | Replace the placeholder with a real eval. |
...
Advisory — weight 0. This check reports **maturity and refactor signals**, not security posture. An 11-stage pipeline isn't a vulnerability; an MCP server with one consumer isn't exploitable. Sub-checks 1–5 are each a judgment call about whether a given artifact has outgrown its current form — decisions that depend on roadmap and team context the scanner can't see. Scoring these would conflate "this project should consider refactoring" with "this project has a security problem," degrading both signals. The check is intentionally a nudging layer on top of the scored moat.

**Sub-check 0 is the exception, and is unscored anyway.** `graduated-script-missing` is not a judgment call — a `status: graduated-code` skill whose script is absent is non-functional (`exit 127` on every invocation) regardless of roadmap or team context. It sits here because it is discovered from the same skill walk as the other sub-checks, not because it shares their advisory character. Consequence to be aware of: because weight is per check, a broken graduated skill currently costs **zero points** and surfaces as visibility only. Making it score requires moving it to its own check or introducing per-finding weights — a deliberate open decision, not an oversight.
...
- **Graduated-script detection is deliberately conservative** — it only inspects skills declaring `status: graduated-code` (a v1.x LLM-driven skill legitimately has no script), and only reports **absolute** paths invoked via `bash`/`sh`/`python`/`python3`/`node`. Skipped without a finding: placeholder or interpolated tokens (`<path>`, `${VAR}`, `$1`, `{{ }}`), comment lines, and **relative paths** — a relative path resolves against the shell's cwd at invocation time, which a static scan cannot know, so guessing a base would manufacture both misses and false hits. `~` and `$HOME` are expanded against the scan's homedir before the existence test.
...
- **An eval directory is coverage only if it carries something runnable or gradeable.** `evals/<name>/` counts when its top level holds an *executable* `test_*.sh` with content (the executable bit is not required on Windows, which has none — same carve-out as `git-hooks`), a `cases.jsonl` with at least one line parsing as a JSON object with `prompt` and `expected_assertions`, or a non-empty `acceptance-criteria.md`. A `tests/test_<name>.*` entry counts only when it has non-whitespace content (a matching directory counts when any file under it does). Anything else that exists — an empty directory, a README-only directory, a zero-byte test file — is reported as `skill-eval-hollow`, not as coverage. Rationale: the earlier form `stat()`ed the directory, so `mkdir` alone turned the check green; a consuming repo ran 17 eval directories holding nothing but a README that said "STUB — presence-check satisfier" while its dashboard reported full coverage. Only the top level of `evals/<name>/` is inspected, so an eval nested a level deeper is not seen.
