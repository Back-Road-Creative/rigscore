...
The table above is a hand-maintained excerpt — it is **not** auto-generated. The authoritative, complete set is whatever `credentialClients()` in `src/clients.js` returns (currently 23 credential-file entries, more than the 10 shown here); add a client there and treat this table as illustrative, not exhaustive.
