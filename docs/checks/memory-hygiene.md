...
Agent memory — the files an agent auto-loads every session — is both a context-budget surface and a correctness surface. Every byte is re-injected on every turn, so bloat is billed per request, and a memory file that says nothing still costs that toll. **No incumbent convention exists for memory layout; this check defines one.** Six signals ship today: **budget** (the auto-loaded bundle exceeds a byte budget, now configurable), **stale content** (a memory file that is empty, or a bare stub with frontmatter and no body), **single home per rule** (the same rule written into both a governance file and a memory file, so editing one copy silently fails to take effect), **unresolvable index entries** (a `MEMORY.md` entry naming a topic file that does not exist, or that lives outside every memory directory — the index promises a memory nothing ever loads), and — once a memory set actually uses `[[wikilinks]]` — a **warn-only wikilink-graph report** (**dangling links** to a memory with no file, and **orphan memories** no other memory links to) that turns manual reconciliation into a surfaced signal. Budget, stale content, and index resolution are pure arithmetic — byte counts, body-length math, and a path that either resolves inside a scanned memory dir or does not. Single-home is exact-match comparison after normalization, against every governance file that is loaded alongside memory — the root set, nested package governance, and (opt-in) home governance — with hard-wrapped lines re-joined first: deterministic, but a proxy for "same rule", so it is tuned to miss rather than to over-call. Maps loosely to OWASP Agentic **ASI01 — Agent Goal Hijack**: memory is injected ahead of the live instruction, so junk in it competes with governance for the model's attention. A pass means memory is small, every file carries a rule, and every rule has exactly one home.
...
| A `[[wikilink]]` names a memory with no matching file (only once the set uses wikilinks) | WARNING | `memory-hygiene/dangling-wikilink` | Write the file, or fix / drop the reference |
| A memory file has no inbound `[[wikilink]]` from any other memory (wikilink-graph orphan) | WARNING | `memory-hygiene/orphan-memory` | Link to it from a related memory, or delete it if dead |
...
- **Index-orphan memory files — owned by another check, deliberately not duplicated.** `workflow-maturity/memory-orphan` flags `.md` files in a memory dir that no `MEMORY.md` links, including the no-index-at-all case. This check's `orphan-memory` is a *different* reconciliation — the `[[wikilink]]` cross-reference graph, not the index — so a file can be indexed (not an index-orphan) yet still have zero inbound wikilinks (a wikilink-graph orphan). Neither subsumes the other.
...
- **`[[wikilink]]` reconciliation is warn-only and convention-gated.** It runs only when the memory set contains at least one `[[wikilink]]` — a repo whose `MEMORY.md` is a plain markdown-link TOC never sees these findings, so the convention is never imposed. `dangling-wikilink` fires per unique missing target (`[[foo]]` with no `foo.md` in scope); `orphan-memory` fires per topic file with zero inbound wikilinks from *other* memory files (the index `MEMORY.md` is excluded, and a self-link does not count as inbound). Both are WARNING, but the check is advisory (weight 0), so they surface for reconciliation and never move the score or block. A hub note that only links outward and is never linked-to counts as an orphan — intentional, since the wikilink graph cannot reach it. Wikilinks in fenced code are samples, not references. Each is capped at 10 findings; `data.danglingWikilinks` / `data.orphanMemories` carry the true counts.
