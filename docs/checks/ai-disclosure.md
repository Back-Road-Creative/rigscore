...
- Tool component: `rigscore`. Rule IDs emitted: the explicit `findingId` per finding — `ai-disclosure/no-ai-policy`, `ai-disclosure/pr-template-no-ai-field`, `ai-disclosure/no-pr-template`. See `src/sarif.js` → `deriveFindingRuleId()`.
