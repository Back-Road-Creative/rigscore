# GEMINI.md — rigscore starter template

Starter governance file for Gemini CLI, calibrated to score well under
rigscore's default profile. Gemini CLI reads `GEMINI.md` for hierarchical
project context; drop this at the repo root and adjust paths, gates, and
commands to match your project. Gemini also reads `AGENTS.md` — if several
agents share the repo, prefer the vendor-neutral `agents-md/` template and
point this file at it with a one-line stub.
