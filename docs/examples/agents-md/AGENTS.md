# AGENTS.md — rigscore starter template

Vendor-neutral instruction file, calibrated to score well under rigscore's
default profile. `AGENTS.md` is the cross-client convention (Codex, Jules,
Cursor, Copilot, Zed, and others read it), so it is the one governance
file worth writing when several agents share one repository. Clients that
prefer their own filename can point at it with a one-line stub.

## Project

- Build: `npm run build` · Test: `npm test` · Lint: `npm run lint`
- Runtime, package manager, and directory layout go here so every agent
  works from the same facts instead of guessing.
...
- Shell restrictions: reserve the terminal for git, docker, and the test
  runner. Read and edit files with the editor tools, not with `sed`.
...
- Never present machine-generated output as if a human wrote it unreviewed;
  a human stays accountable for every change.
