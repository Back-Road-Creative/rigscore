 ...
 * OWN first-party `claude -p` (Max-plan CLI — never an API key, never an SDK)
 * to read each MCP tool description and classify it benign vs. suspicious.
 *
 ...
 *   - FIRST-PARTY LLM ONLY. The call shells out to the `claude` binary via
 *     execSafe (child_process). If `claude` is not on PATH — or the call errors
 *     or times out — execSafe returns null and the tool is skipped: no finding,
 *     no throw, no hard error.
 ...
 * (benign), or null (judge unavailable — skip this tool gracefully).
 */
async function judge(name, description, runner) {
  const stdout = await runner('claude', ['-p', buildJudgePrompt(name, description)], { timeout: JUDGE_TIMEOUT_MS });
...
export default {
  ...
  async run(context) {
    ...
    for (const tool of tools) {
      const verdict = await judge(tool.name, tool.description, runner);
