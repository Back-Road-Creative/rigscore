...
- **Local filesystem only** in default mode. rigscore reads files under the scan target and, across a registry of 29 known agent clients (24 with MCP config paths, 21 holding 23 credential files), the corresponding `$HOME` paths (e.g. `~/.claude/settings.json`, `~/.claude/skills/`). It writes `.rigscore-state.json` into the scan target.
